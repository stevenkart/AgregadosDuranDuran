...
{
    public partial class FrmRevBillProvider : Form
    {
        //variables del form
        ...
        private void FrmRevBillProvider_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        private void Filtrar() // filtra la informacion de factura proveedores compra
        {
            btnReversar.Visible = false;
            ...
            if (txtConsecutivo.Text.Length > 0)
            {
                ...
                var result = from fa in DB.Facturas
                             ...
                             where ((fa.IdEstado == 4 || fa.IdEstado == 3) && fa.IdProveedor > 0 && fa.FechaFactura == fechaActual && fa.Consecutivo == consec && fa.IdCierreApert == apert)
                             select new
                             {
                                 fa.IdFactura,
                                 fa.Consecutivo,
                ...
                else
                {
                    ...
                    btnReversar.Visible = false;
                    MessageBox.Show("No hay Facturas de Compra disponibles por Reversar.",
            ...
            else
            {
                if (txtClienteNombre.Text.Length > 0)
                {
                    ...
                    var result = from fa in DB.Facturas
                                 ...
                                 join pr in DB.Proveedores on fa.IdProveedor equals pr.IdProveedor
                                 join us in DB.Usuarios on fa.IdUsuario equals us.IdUsuario
                                 where ((fa.IdEstado == 4 || fa.IdEstado == 3) && fa.IdProveedor > 0 && fa.FechaFactura == fechaActual && pr.Nombre.Contains(cliente) && fa.IdCierreApert == apert)
                                 select new
                                 {
                                     fa.IdFactura,
                                     fa.Consecutivo,
                ...
                else
                {
                    ...
                    var result = from fa in DB.Facturas
                                 ...
                                 join pr in DB.Proveedores on fa.IdProveedor equals pr.IdProveedor
                                 join us in DB.Usuarios on fa.IdUsuario equals us.IdUsuario
                                 where ((fa.IdEstado == 4 || fa.IdEstado == 3) && fa.IdProveedor > 0 && fa.FechaFactura == fechaActual && fa.IdCierreApert == apert)
                                 select new
                                 {
                                     fa.IdFactura,
                                     fa.Consecutivo,
        ...
        private void btnReversar_Click(object sender, EventArgs e)
        {
            try
            {
                if (Consecutivo > 0)
                {
                    ...
                    {
                        DialogResult respuesta = MessageBox.Show("¿Deseas continuar con la reversión de la factura de Compra seleccionada?",
                        ...
                        if (respuesta == DialogResult.Yes)
                        {
                            ...
                            if (facturas.IdTipo == 1) // contado
                            {
                                if (facturas.IdTipoPago == 1) //efectivo
                                {
                                    ...
                                    if (DB.SaveChanges() > 0)
                                    {
                                        MessageBox.Show("Factura de Compra fue reversada correctamente!",
                                ...
                                else
                                {
                                    if (facturas.IdTipoPago == 2) //trans sinpe
                                    {
                                        ...
                                        //reversamos datos financieros del cierre de caja
                                        cierreApertCajas = DB.CierreApertCajas.Find(facturas.IdCierreApert);
                                        cierreApertCajas.MontoCompraTransf -= facturas.CostoTotal;
                                        DB.Entry(cierreApertCajas).State = EntityState.Modified;
                                    ...
                                    else
                                    {
                                        if (facturas.IdTipoPago == 3) // sinpe movil
                                        {
                                            ...
                                            //reversamos datos financieros del cierre de caja
                                            cierreApertCajas = DB.CierreApertCajas.Find(facturas.IdCierreApert);
                                            cierreApertCajas.MontoCompraSinpe -= facturas.CostoTotal;
                                            DB.Entry(cierreApertCajas).State = EntityState.Modified;
                            ...
                            else
                            {
                                if (facturas.IdTipo == 2) // credito
                                {
                                    ...
                                    //reversamos datos financieros del cierre de caja
                                    cierreApertCajas = DB.CierreApertCajas.Find(facturas.IdCierreApert);
                                    cierreApertCajas.MontoCompraCredito -= facturas.CostoTotal;
                                    DB.Entry(cierreApertCajas).State = EntityState.Modified;
                                    ...
                                    {
                                        MessageBox.Show("No se pudo reversar factura de Compra, correctamente, favor validar",
                                                                "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                                    }
                                }
                            }
                    ...
                    {
                        MessageBox.Show("Factura de Compra no esta disponible para Reversa ya que apertura caja no corresponde a dicha factura, favor validar detalle de la factura.",
                                                     "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
                else
                {
                    MessageBox.Show("Factura de Compra no fue seleccionada correctamente, por favor valida la información en la tabla.",
                                                   "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            catch (Exception)
