...
namespace Agregados.Forms.Bills
{
    public partial class FrmRevFacts : Form
    {
        ...
        {
            MessageBox.Show("Este apartado permite anular una factura que se haya emitido anteriormente, en el mismo día." + Environment.NewLine
                + "No es posible anular o reversar una factura de días anteriores al actual.",
        ...
        private void Filtrar() // filtra la informacion
        {
            ...
            if (txtConsecutivo.Text.Length > 0)
            {
                ...
                var result = from fa in DB.Facturas
                             ...
                             where (fa.IdEstado == 4 && fa.IdCliente > 0 && fa.FechaFactura == fechaActual && fa.Consecutivo == consec)
                             select new
                             {
                                 fa.IdFactura,
                                 fa.Consecutivo,
            ...
            else
            {
                if (txtClienteNombre.Text.Length > 0)
                {
                    ...
                    var result = from fa in DB.Facturas
                                 ...
                                 join cl in DB.Clientes on fa.IdCliente equals cl.IdCliente
                                 join us in DB.Usuarios on fa.IdUsuario equals us.IdUsuario
                                 where (fa.IdEstado == 4 && fa.IdCliente > 0 && fa.FechaFactura == fechaActual && cl.Nombre.Contains(cliente))
                                 select new
                                 {
                                     fa.IdFactura,
                                     fa.Consecutivo,
